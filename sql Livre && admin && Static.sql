...
SELECT COUNT(*) FROM gestionbib.Livres;

CREATE DATABASE gestionbib;
drop table membres;
SELECT * FROM gestionbib.Livres;
DELETE FROM Livres WHERE id_livre=74;
...
('Ahmed El Mansouri', 'Rue Mohammed V, Casablanca', 0661234567),
('Fatima Ben Salah', 'Avenue Hassan II, Rabat', 0612345678),
('Youssef Alaoui', 'Boulevard Mohamed VI, Marrakech', 0633456789),
('Nora Chaoui', 'Avenue Mohammed V, Fes', 0644567890);
...
INSERT INTO gestionbib.adminn (name_ad, password) 
VALUES ('taha', 'taha');
SELECT * FROM gestionbib.adminn;
DELETE FROM adminn WHERE id_admin=2;
...
create table statistic (
   id_statistic int not null primary key auto_increment,
   Livres int,
   Emprunts int,
  Membres int 
);
INSERT INTO gestionbib.Livreadminns (titre, lauteur, lannéepublication) 
VALUES 
('Carrie', 'Stephen King', 1974);